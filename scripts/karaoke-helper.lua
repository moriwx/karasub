...
UI_conf = {
	wchar_dialogs = {
		...
		{ 1, 0, 1, 1, "dropdown", name = "tag", items = {"\\kf", "\\k", "\\ko"} , value = "\\k" }
	},
    ...
    for i = 1, #subtitles do
        ...
        if check_subtitle_line(line, handle_style) then -- 判断该行字幕是否需要进行卡拉OK样式处理
            ...
            if not last_line then -- 每层歌词的首行
                ...
                new_start_time = last_line.end_time + 200 -- 空200ms
...
function set_style_3(subtitles, handle_style, handle_chord_style, advance_time, extend_time, sep_threshold, countdown, countdown_style)
    local layers = {}  -- 使用 table 记录各层最后一行信息（包含索引）
    local layer_i = 0
    local first_lines = {}
    local first_line_advanced_time = {}
    ...
    for i = 1, #subtitles do
        ...
        if check_subtitle_line(line, handle_style) then
            local layer_info = layers[layer_i]
            local new_start_time

            -- 判断是否是该层首行
            if not layer_info then
                if layer_i == 0 then  -- 新小节首行
                    new_start_time = line.start_time - advance_time
                    if new_start_time < 0 then new_start_time = 0 end
                    table.insert(first_lines, i)
                    table.insert(first_line_advanced_time, line.start_time - new_start_time)
                else  -- 其他层首行取最大结束时间，需修改为0的开始时间
                    new_start_time = layers[0].line.start_time
                end
            else  -- 非首行按层尾计算
                new_start_time = layer_info.line.end_time + 400
            end

            -- 检查是否需要分割小节
            local max_end_all = 0
            for _, info in pairs(layers) do
                if info.line.end_time > max_end_all then
                    max_end_all = info.line.end_time
                end
            end
            if line.start_time >= max_end_all + sep_threshold then
                -- 延长前一结尾
                local last_info
                for _, info in pairs(layers) do
                    if info.line.end_time == max_end_all then
                ...
                if last_info then
                    last_info.line.end_time = last_info.line.end_time + extend_time - 400
                    last_info.line.text = string.format("%s{\\k%d}", last_info.line.text, 
                        math.floor((extend_time - 400)/10))
                    subtitles[last_info.index] = last_info.line
                end
            ...
            -- 更新行数据
            line.text = string.format("{\\k%d}%s{\\k40}", 
                math.floor((line.start_time - new_start_time)/10), line.text)
            line.start_time = new_start_time
            line.end_time = line.end_time + 400
            line.style = string.format("K%d", layer_i + 1)
            subtitles[i] = line

            -- 更新层状态
            layers[layer_i] = {line = line, index = i}
            layer_i = (layer_i + 1) % 3  -- 改为三阶循环
        end
    end
    ...
    local last_info
    for _, info in pairs(layers) do
        if info.line.end_time > max_end then
            max_end = info.line.end_time
            last_info = info
        end
    end
    if last_info then
...
function macro_set_style_3(subtitles, selected_lines)
	local inum, info_table, style_table = read_infos(subtitles, selected_lines)
	local styles = {}
	for i = 1, #style_table do
		if style_table[i] then
    ...
    btn, result = aegisub.dialog.display(config)
    if btn then
        set_style_3(subtitles, result.style, result.chord_style, math.floor(result.advance_time), math.floor(result.extend_time), math.floor(result.sep_threshold), result.countdown, result.countdown_style)
    end
end
...
TLL_macros = {
	...
	{
		script_name = "转换为三层交错形式",
		script_description = "将连续的字幕转换为K1-K3三层交错的形式",
		entry = function(subs,sel) macro_set_style_3(subs, sel) end,
		validation = false
	},
	{
